...
//    private boolean mFullscreenEnabled = false;
    ...
    private ImageButton mTurnButton;// 寮�鍚殏鍋滄寜閽�
...
//        mTitleLayout = v.findViewById(R.id.title_part);
        ...
        lockIv = (ImageView) v.findViewById(R.id.lock);
        if (mTurnButton != null) {
        ...
        if(lockIv != null){
            lockIv.setOnClickListener(mLockListener);
        }

        if (mBackButton != null) {//杩斿洖鎸夐挳浠呭湪鍏ㄥ睆鐘舵�佷笅鍙
...
//        mTitle = (TextView) v.findViewById(R.id.title);
     ...
     */
    public void show(int timeout) {//鍙礋璐ｄ笂涓嬩袱鏉ar鐨勬樉绀�,涓嶈礋璐ｄ腑澶甽oading,error,playBtn鐨勬樉绀�.
        Log.d("Controller", "show: " + timeout);
        if (!mShowing) {
            setProgress();
            if (mTurnButton != null) {
                mTurnButton.requestFocus();
...
//        }
        if(mBackButton.getVisibility() != VISIBLE && !mLock){
        	mBackButton.setVisibility(VISIBLE);
        }
        if (mControlLayout.getVisibility() != VISIBLE && !mLock) {
            mControlLayout.setVisibility(VISIBLE);
        }
        if(lockIv.getVisibility() != VISIBLE) {
            lockIv.setVisibility(VISIBLE);
        }
    ...
    public void hide() {//鍙礋璐ｄ笂涓嬩袱鏉ar鐨勯殣钘�,涓嶈礋璐ｄ腑澶甽oading,error,playBtn鐨勯殣钘�
        if (mShowing) {
            mHandler.removeMessages(SHOW_PROGRESS);
//            mTitleLayout.setVisibility(GONE);
            mBackButton.setVisibility(GONE);
            mControlLayout.setVisibility(GONE);
            lockIv.setVisibility(GONE);
            mShowing = false;
        }
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                Log.d("Controller", "onTouchEvent down : " + mLock);
                if(isShowing()){
                    hide();
                    handled = true;
                }else {
            ...
            case MotionEvent.ACTION_UP:
                Log.d("Controller", "onTouchEvent up: " + handled);
                if (!handled) {
                    handled = false;
...
//        if(mIsFullScreen){
//        	RelativeLayout.LayoutParams lp = (RelativeLayout.LayoutParams) mBackButton.getLayoutParams();
//        	lp.topMargin = CommUtils.dip2px(getContext(), 50);
...
//    }
    ...
    private OnClickListener mLockListener = new OnClickListener() {
        ...
        public void onClick(View v) {
            mLock = !mLock;
            if (mLock) {
                lockIv.setImageResource(R.drawable.ic_lock_black_24dp);
            }else {
                lockIv.setImageResource(R.drawable.ic_lock_outline_black_24dp);
            }
            mBackButton.setVisibility(GONE);
            mControlLayout.setVisibility(GONE);
            show(sDefaultTimeout);
...
//    	System.out.println("updateScaleButton:" + mIsFullScreen);
//        if (mIsFullScreen) {
